...
use Aprilo\Cashback\Helper\Data as DataHelper;
use Aprilo\Cashback\Model\Cashback\Action as CashbackAction;
...
class Transaction extends AbstractModel implements IdentityInterface
{
     ...
     * Cache
     */
    const CACHE_TAG = 'Aprilo_cashback_transaction';
    ...
    public function __construct(
        Context $context,
        Registry $registry,
        \Aprilo\Cashback\Model\CreditFactory $creditFactory,
    ...
    {
        parent::_construct();
        $this->_init(\Aprilo\Cashback\Model\ResourceModel\Transaction::class);
    }
    ...
    protected function prepareTransaction($action, $amount, &$customer, $extraContent)
    {
        ...
        $balanceAfterChange = 0;
        if ((int) $action === Action::ACTION_PENDING) {
            $balanceAfterChange = $credit->getBalance();
        }else{
        ...
        if ((int) $action === Action::ACTION_EXPIRED) {
            $credit->setAmount($amount);
            $credit->setAction(Action::ACTION_EXPIRED);
            $credit->setOrderIncrementId(0);
            $credit->setCustomer($customer);
        }
    ...
    public function getTransactionsForCustomer($customerId)
    {
        ...
        $transactions = $this->getCollection()->setOrder('transaction_date', 'desc');
        $transactions->getSelect()
            ->join(
                ['cr' => $transactions->getTable('Aprilo_cashback_credit')],
    ...
    public function afterSave()
    {
        ...
        //                 'title'            => (String)Action::getActionLabel($this->getAction(), $this->getExtraContent()),
